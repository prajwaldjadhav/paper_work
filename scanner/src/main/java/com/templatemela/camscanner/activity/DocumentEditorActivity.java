...
import com.templatemela.camscanner.adapter.ColorEffectAdapter;
import com.templatemela.camscanner.adapter.ColorFilterAdapter;
import com.templatemela.camscanner.adapter.EditToolsAdapter;
import com.templatemela.camscanner.adapter.FontAdapter;
import com.templatemela.camscanner.adapter.OverlayAdapter;
import com.templatemela.camscanner.adapter.SignatureAdapter;
import com.templatemela.camscanner.adapter.WatermarkFontAdapter;
import com.templatemela.camscanner.db.DBHelper;
import com.templatemela.camscanner.document_view.ColorFilter;
...
import com.templatemela.camscanner.models.EditToolType;
...
public class DocumentEditorActivity extends BaseActivity implements View.OnClickListener, EditToolsAdapter.OnToolSelected, SeekBar.OnSeekBarChangeListener {
    ...
    //store index of the selected adjust
    //0-> brightness
    //1 -> contrast
    //2-> saturation
    //3-> exposure
...
//    private AspectRatioLayout ly_aspectratio;
    ...
    //will store the list of signature
...
//        ly_aspectratio.setAspectRatio((float) Constant.original.getWidth(), (float) Constant.original.getHeight());
        ...
        //building a photo editor and set false to disable pinch to zoom on text insertion. Default: true.
        photoEditor = new PhotoEditor.Builder(this, iv_editImg).setPinchTextScalable(true).build();

        //setting a image to the photo editer view
        iv_editImg.getSource().setImageBitmap(Constant.original);

        //setting a tools in recycle view
        setEditToolsAdapter();

        //initialy selected filter
        ...
        //initial value of seekbar(adjust)
        ...
        //drawing false on photoEditor view
        ...
        //watermark
        watermarkFont = R.font.roboto_medium;//setting default text
        //changing the color of the text in edit box
    ...
    //setting adapter for recycle view containing all the edit tools
    ...
    //when any of the edit tool is selected
    @Override
    public void onToolSelected(EditToolType editToolType) {
        switch (editToolType) {
            ...
            case HIGHLIGHT:
                ...
                //select the highlighter and deselect the other tab
            ...
            case WATERMARK:
                ...
                return;
            case TEXT:
                //open the dailog box where the text to be added will be visible
                addTextDialog("", false);
                return;
            case COLOREFFECT:
    ...
    //get the image from the imagepicker library and add the image to the stickerview.
    @Override
    ...
    public void onClick(View view) {
        int id = view.getId();
        if (id == R.id.iv_align_center) {//select the center and deselect the other tab in alignment
            ...
            return;
        } else if (id == R.id.iv_align_left) {//select the left tab and deselect the other tab in a alignment
            ...
            ChangeTextSticker(defaultTxtColor, this, stickerView);
            return;
        } else if (id == R.id.iv_align_right) {//select the right tab and deselect the other tab in a alignment.
            iv_align_left.setImageResource(R.drawable.tic_left);
            iv_align_center.setImageResource(R.drawable.tic_center);
            ...
            ChangeTextSticker(defaultTxtColor, this, stickerView);
            return;
        } else if (id == R.id.iv_apply_adjust) {//apply adjust effect
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            slideDownAnimation(ly_adjust);
            return;
        } else if (id == R.id.iv_apply_effect) {//apply color effect
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            slideDownAnimation(ly_color_effect);
            return;
        } else if (id == R.id.iv_apply_filter) {//apply color filter
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            slideDownAnimation(ly_color_filter);
            return;
        } else if (id == R.id.iv_apply_highlight) {//apply highlight
            photoEditor.setBrushDrawingMode(false);
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            slideDownAnimation(ly_highlight);
            return;
        } else if (id == R.id.iv_apply_opacity) {//apply opacity
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            slideDownAnimation(ly_opacity);
            return;
        } else if (id == R.id.iv_apply_overlay) {//apply overlay
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            slideDownAnimation(ly_overlay);
            return;
        } else if (id == R.id.iv_apply_signature) {//apply signature
            //if signature view is not empty then extract a bitmap from it.
            //save signature to the image using async task.
            if (!signature_view.isBitmapEmpty()) {
                //get the bitmap to the signature view
                Bitmap signatureBitmap = signature_view.getSignatureBitmap();
                //store the bitmap to the external file using async task and postexecution of the async task run the
                new saveSignature().execute(new Bitmap[]{signatureBitmap});
                return;
            }
            return;
        } else if (id == R.id.iv_apply_txt) {//apply a sticker text to the image
            ...
            return;
        } else if (id == R.id.iv_apply_watermark) {
        ...
        if (id == R.id.iv_bold) {//if the bold is selected earlier then deselect it.
            if (isbold) {
                iv_bold.setImageResource(R.drawable.tic_bold);
                if (isitalic) {
                    //if text is not bold and but it is italic
                    ChangeTextAlignment(Typeface.create(defaultFont, Typeface.ITALIC), defaultAlign);
                    isbold = false;
                    return;
                }
                //if text is not bold and not italic then set the default font
            ...
            isbold = true;
            return;
        } else if (id == R.id.llBrightness) {//select the brightness and deselect the other tab
        ...
        if (id == R.id.iv_close_adjust) {//
            Constant.filterPosition = 0;

            //when click on the close button then reset the adjust tab and restore the original image
            iv_brightness.setImageResource(R.drawable.ic_brightness_selection);
            iv_contrast.setImageResource(R.drawable.ic_contrast);
            iv_saturation.setImageResource(R.drawable.ic_saturation);
            iv_exposure.setImageResource(R.drawable.ic_exposure);
            ...
            slideDownAnimation(ly_adjust);
            return;
        } else if (id == R.id.iv_close_effect) {//when click on close button of the color effect then we have to restore the original image
            iv_editImg.getSource().setColorFilter(Constant.coloreffect[0]);
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            slideDownAnimation(ly_color_effect);
            return;
        } else if (id == R.id.iv_close_filter) {//when click on the close button of the color filter then we have to restore the original image
            Constant.filterPosition = 0;
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            slideDownAnimation(ly_color_filter);
            iv_editImg.getSource().setImageBitmap(Constant.original);
            return;
        } else if (id == R.id.iv_close_highlight) {
            photoEditor.setBrushDrawingMode(false);
            ...
            slideDownAnimation(ly_highlight);
            return;
        } else if (id == R.id.iv_close_opacity) {//when click on the close button of the opacity then set the opacity to 255 to get the original image(no blurness)
            ...
            slideDownAnimation(ly_opacity);
            return;
        } else if (id == R.id.iv_close_overlay) {
        ...
        if (id == R.id.iv_close_signature) {
            signature_view.clearCanvas();
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            rl_signature.setVisibility(View.GONE);
            return;
        } else if (id == R.id.iv_close_txt) {
            ...
            slideDownAnimation(ly_text);
            return;
        } else if (id == R.id.iv_close_watermark) {
            ly_edit_tools.setVisibility(View.VISIBLE);
            iv_done.setVisibility(View.VISIBLE);
            rv_watermark_font.setVisibility(View.GONE);
            rl_watermark_color.setVisibility(View.GONE);
            ...
            rl_watermark.setVisibility(View.GONE);
            return;
        } else if (id == R.id.llColor) {//color picker dialog
            ColorPickerDialogBuilder.with(this).setTitle("Choose color").initialColor(brush_color).wheelType(ColorPickerView.WHEEL_TYPE.FLOWER).density(10).setOnColorSelectedListener(new OnColorSelectedListener() {
                ...
                public void onClick(DialogInterface dialogInterface, int i, Integer[] numArr) {
                    ...
                    //on click on ok update the brush color and select the highlight tab again.
                    brush_color = i;
        ...
        if (id == R.id.llContrast) {
            Constant.filterPosition = 0;

            //make contrast tab as active and make other deactive
            ...
            tv_progress.setText(Constant.getProgressPercentage(sb_adjust.getProgress(), 255) + "");
            return;
        } else if (id == R.id.llCreateSig) {//active the signature tab and deselect the saveed sig tab
            iv_create_signature.setImageResource(R.drawable.ic_create_sig_selection);
            iv_saved_signature.setImageResource(R.drawable.ic_saved_sig);
            ...
            rl_signature_list.setVisibility(View.GONE);
            return;
        } else if (id == R.id.iv_done) {//set constrained
            stickerView.setConstrained(false);
            stickerView.setLocked(true);
            //when click on the saved button  then apply the changes
            new saveFinalBitmap().execute(new Bitmap[0]);
            return;
        } else if (id == R.id.llEraser) {//activate a eraser tab and deactivate othere
            ...
            //convert brush to eraser
            photoEditor.brushEraser();
            return;
        } else if (id == R.id.llExposure) {//activate the exposure tab and deselect the other
            iv_brightness.setImageResource(R.drawable.ic_brightness);
            iv_contrast.setImageResource(R.drawable.ic_contrast);
            iv_saturation.setImageResource(R.drawable.ic_saturation);
            ...
            tv_progress.setText(Constant.getProgressPercentage(sb_adjust.getProgress(), 255) + "");
            return;
        } else if (id == R.id.llHighLight) {//highlighter pen
            //select this as active tab and deactivate other
            iv_highlight.setImageResource(R.drawable.bic_highlight_selection);
            iv_erase.setImageResource(R.drawable.bic_erase);
            ...
            sb_highlight_size.setVisibility(View.VISIBLE);
            sb_eraser_size.setVisibility(View.GONE);
            //convert the brush to drawing mode
            photoEditor.setBrushDrawingMode(true);
            return;
            ...
            if (isitalic) {
                iv_italic.setImageResource(R.drawable.tic_italic);
                if (isbold) {
                    //text was italic so this time it is not italic
                    //not italic and bold
                    ChangeTextAlignment(Typeface.create(defaultFont, Typeface.BOLD), defaultAlign);
                    isitalic = false;
                    return;
                }
                //text was italic and now it will be not be italic
                //even its is not bold
                ChangeTextAlignment(defaultFont, defaultAlign);
                isitalic = false;
                return;
            }
            ...
            //text was not italic now it will be converted to italic
            ...
            isitalic = true;
            return;
        } else if (id == R.id.llSaturation) {
            Constant.filterPosition = 0;

            //select the saturation tab and deselect the other
            iv_brightness.setImageResource(R.drawable.ic_brightness);
            iv_contrast.setImageResource(R.drawable.ic_contrast);
            ...
            tv_progress.setText(Constant.getProgressPercentage(sb_adjust.getProgress(), 255) + "");
            return;
        } else if (id == R.id.llSavedSig) {//click on saved signature-> show all the saved signature
            setSignatureAdapter();
            //activate the saved tab and deselect the other tab
            ...
            return;
        } else if (id == R.id.tv_clear_signature) {
            signature_view.clearCanvas();
            return;
        }
        if (id == R.id.iv_txt_alignment) {//text aligbnment button will pop up option(left/right/center/bold/italic)
            rv_font.setVisibility(View.GONE);
            rl_txt_color.setVisibility(View.GONE);
            ly_alignment.setVisibility(View.VISIBLE);
            return;
        } else if (id == R.id.iv_txt_color) {//text color option will pop up the option (color seekbar)
            rv_font.setVisibility(View.GONE);
            rl_txt_color.setVisibility(View.VISIBLE);
            ly_alignment.setVisibility(View.GONE);
            return;
        } else if (id == R.id.iv_txt_font) {//select the font
            ...
            //visible the font and hide other
            rv_font.setVisibility(View.VISIBLE);
            rl_txt_color.setVisibility(View.GONE);
            ly_alignment.setVisibility(View.GONE);
            return;
        }
        if (id == R.id.iv_watermark_color) {
            rv_watermark_font.setVisibility(View.GONE);
            rl_watermark_color.setVisibility(View.VISIBLE);
            rl_watermark_opacity.setVisibility(View.GONE);
            return;
        } else if (id == R.id.iv_watermark_font) {
            ...
            rl_watermark_opacity.setVisibility(View.GONE);
            return;
        } else if (id == R.id.iv_watermark_opacity) {
            rv_watermark_font.setVisibility(View.GONE);
            rl_watermark_color.setVisibility(View.GONE);
...
//        }
        ...
        //inputMethodManager(keypad on phone)
...
//        editText.setTextColor(defaultTxtColor);
        ...
        textView2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (editText.getText().toString().trim().length() >= 1) {
                    ...
                    if (isText) {
                        //if text was present previously then change the text of the sticker
                        ...
                        //if text was present previously the add a new sticker.
                        defaultText = editText.getText().toString();
                        dialog.dismiss();
        ...
        textView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                //hide the text edit state and visible the edit tools
    ...
    //addTextSticker(color,activity,stickerview)
    //text to be added to sticker is store in default text
    ...
    //changeTextSticker(color,activity,stickerview)
    public void ChangeTextSticker(int i, Activity activity, StickerView stickerView2) {
        try {
            ...
            textSticker.setTextAlign(defaultAlign);
            textSticker.resizeText();
            //replace the current selected sticker
    ...
    //passing colorFilter to the photoEditor view
    ...
    public void onProgressChanged(SeekBar seekBar, int i, boolean z) {
        ...
        if (id == R.id.sb_adjust) {//brightness
            if (adjustPosition == 0) {
        ...
        } else if (id == R.id.sb_eraser_size) {
            photoEditor.setBrushEraserSize((float) i);
            photoEditor.brushEraser();
            return;
        } else if (id == R.id.sb_highlight_size) {
    ...
    //change the contrast of the image present in photoeditor view
    ...
    //change the exposure of the image present in photoeditor view
    ...
    private void setSignatureAdapter() {
        signatureList.clear();
        //set the signature list (with path of the signature previously store)
        setSignatureList();
    ...
    //initialize thet signatureListArray
    ...
    //given bitmap as input
    //store the bitmap in external storage in /signature folder
    //on postexecute add the bitmap to the sticker
    ...
    private class saveFinalBitmap extends AsyncTask<Bitmap, Void, Bitmap> {
        ...
        @Override
        public Bitmap doInBackground(Bitmap... bitmapArr) {
            //creating a bitmap from relative layout ( containing -> photoeditor view / sticker view / overlayview)
            ...
            if (fromTAG.equals("SavedDocumentActivity")) {
                ...
                if (Constant.inputType.equals("Group")) {
                    ...
                    //replace the previouse image with current image
        ...
        public void onPostExecute(Bitmap bitmap) {
            ...
            if (fromTAG.equals("IDCardPreviewActivity")) {
                ...
                AdsUtils.jumpNextActivity(DocumentEditorActivity.this);
